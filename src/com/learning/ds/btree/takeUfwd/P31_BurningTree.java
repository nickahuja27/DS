...
public class P31_BurningTree {
    ...
    public static boolean burningTree(TreeNode<Integer> node, int targetNum, Queue<TreeNode<Integer>> queue) {
        if(node == null)
            return false;
        ...
        boolean leftFound = burningTree(node.left, targetNum, queue);
        if(leftFound) {
            while (!queue.isEmpty()) {
                TreeNode<Integer> tempNode = queue.poll();
                System.out.println(tempNode.value + " ");
        ...
        boolean rightFound = burningTree(node.right, targetNum, queue);
        if(rightFound) {
            while (!queue.isEmpty()) {
                TreeNode<Integer> tempNode = queue.poll();
                System.out.println(tempNode.value + " ");
