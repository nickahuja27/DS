...
public class P28_ShortestPathInUnDirectedGraph {
  ...
  private static void findShortedPath(ArrayList<ArrayList<Integer>> adjList) {
    ...
    while (!queue.isEmpty()) {
      ...
      ArrayList<Integer> neighbors = adjList.get(tempNode);
      for(int i = 0; i < neighbors.size(); i++) {
        int neighbor = neighbors.get(i);
        int oldDistance = distance[neighbor];
        int newValue = distance[tempNode] + 1;
        distance[neighbor] = Math.min(oldDistance, newValue);
