...
public class P26_RootToNodePath {
    ...
    private static void print(TreeNode<Integer> node, int nodeToFind, String pathSoFar) {
        if(node == null) return;
        if(node.value == nodeToFind) System.out.println(pathSoFar + node.value);
        print(node.left, nodeToFind, pathSoFar + node.value);
        print(node.right, nodeToFind, pathSoFar + node.value);
    }
