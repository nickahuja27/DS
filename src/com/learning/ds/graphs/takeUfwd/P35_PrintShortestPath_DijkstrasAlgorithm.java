...
public class P35_PrintShortestPath_DijkstrasAlgorithm {
   ...
   static int[][] edges = {
    ...
    private static void findShortestPath(ArrayList<ArrayList<WeightedEdge>> adjList) {
        distance[SRC_NODE] = 0;
        PriorityQueue<WeightedEdge> pQueue = new PriorityQueue<>((edge1, edge2) -> edge1.weight - edge2.weight);
        pQueue.add(new WeightedEdge(SRC_NODE, 0));
        while (!pQueue.isEmpty()) {
            WeightedEdge tempNode = pQueue.poll();
            if(!visited[tempNode.nodeValue]) {
                ...
                for (WeightedEdge neighbor : neighbors) {
                    ...
                    if(!visited[neighbor.nodeValue])
                        pQueue.add(new WeightedEdge(neighbor.nodeValue, neighbor.weight));
                }
            }
        }
    }
