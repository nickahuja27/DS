...
public class P11_CheckCycle_BFS_UnDirectedGraph {
    ...
    public static void findCycleBFS(GraphNode<Integer> node) {
        ...
        while (!queue.isEmpty()) {
            ...
            for(GraphNode<Integer> graphNode : workingNode.node.neighbours) {
                if(graphNode.value != workingNode.parentNode.value && !visited[graphNode.value]) {
                    visited[graphNode.value] = true;
                    queue.add(new WorkingNode<>(graphNode, workingNode.parentNode));
                } else if (visited[graphNode.value]) {
